extern crate susudb;
use crate::susudb::traits::SusuDatabaseTrait;
...
fn add_element(database: susudb::SusuDatabase, matches: &clap::ArgMatches<'static>) {
    let key = matches.value_of("KEY").unwrap();
    let value = matches.value_of("VALUE").unwrap();

    let result = database.add(susudb::SusuData {
        key: key.to_string(),
...
fn get_value(database: susudb::SusuDatabase, matches: &clap::ArgMatches<'static>) {
    let key = matches.value_of("KEY").unwrap();
